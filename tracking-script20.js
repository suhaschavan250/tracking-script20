(function () {
  ...
  function getConfigFromQuery() {
    ...
    const trackingScript = Array.from(scripts).find(s => s.src && s.src.includes('tracking-script20'));
  ...
  function handleClick(event) {
    ...
    if (CONFIG.ctaMethod === 'CTA Button Id') {
      ...
      const ancestorId = getAncestorWithId(clickedElement);
      console.log('[Click] Closest Ancestor ID:', ancestorId);
      console.log('[Click] Expected CTA IDs:', CONFIG.ctaValues);

      const matchedId = ancestorId && CONFIG.ctaValues.includes(normalize(ancestorId));
      console.log(`[Click] CTA Button ID match: ${matchedId}`);

      if (matchedId) {
        console.log('[Tracking] CTA matched by Button ID:', clickedText);
        sendToAllPlatforms('any_cta', clickedText);
      } else {
        console.log('[Tracking] CTA Button ID did NOT match.');
      }
    }
  }
